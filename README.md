...
- **Трекер приглашений** — считает, кто кого привёл: лог каждого входа/выхода в отдельный канал («📥 X зашёл по приглашению Y — теперь у него N»), `/invitetop` — топ за последние 7 дней (только для админов) и еженедельная автопубликация топа в заданный канал. Один человек засчитывается один раз: выход и повторный заход счётчик не увеличивают (в логе при этом видно, кто вернулся и по чьему приглашению). Понимает ванити-ссылку сервера; для работы боту нужно право «Управление сервером».
